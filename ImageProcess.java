...
public class ImageProcess {
     ...
     * @param out
     */
    public void YUV420ToARGB8888Front(
            byte[] yData,
            byte[] uData,
            byte[] vData,
            int width,
    ...
    static void PrintMatrix(boolean[][] matrix, int[] points){
        int rows = matrix.length;
        int columns = matrix[0].length;
        int index = 0;
        for(int row = 0; row < rows; row++){
            for(int column = 0; column < columns; column++){
                if(row == points[1] && column == points[0]) {
                    System.out.print("@ ");
                    index++;
                    continue;
                }
                boolean value = matrix[row][column];
                index++;
